...
beforeAll(async () => {
  ...
  const userIdComment = await UserModel.findOne({email:userComment.email}).then((user)=>{
    return user._id.toHexString();
  })
  post1.user = userId;
  const responsePost = await request(app).post("/posts/addPost").send(post1).set('Authorization', `JWT ${accessToken}`);
...
test('TEST 1: Create Comment - post not found : /posts/comments/:id/createComment', async () => {
  ...
  test('TEST 4: GET All Comments : /posts/comments/AllComments', async () => {
    const response = await request(app).get(`/posts/comments/AllComments`).set('Authorization', `JWT ${accessTokenComment}`);
    expect(response.status).toBe(200);
